...
//查询表，支持多个KEY遍历
func (base *PostgresBase) table(name string) (Map) {
	keys := []string{
...
func (base *PostgresBase) view(name string) (Map) {
	keys := []string{
		fmt.Sprintf("%s.%s", base.name, name),
		fmt.Sprintf("*.%s", name),
		name,
...
func (base *PostgresBase) model(name string) (Map) {
	keys := []string{
		fmt.Sprintf("%s.%s", base.name, name),
		fmt.Sprintf("*.%s", name),
		name,
...
//ID生成器
func (base *PostgresBase) Serial(key string) (int64) {

	exec,err := base.begin()
	if err != nil {
		base.error("data.serial", err, key)
		return 0
	}
...
//获取表对象
func (base *PostgresBase) Table(name string) (DataTable) {
...
//获取模型对象
func (base *PostgresBase) View(name string) (DataView) {
...
//获取模型对象
func (base *PostgresBase) Model(name string) (DataModel) {
...
//注意，此方法为实际开始事务
func (base *PostgresBase) begin() (PostgresExecutor, error) {
	if base.manual {
		if base.tx == nil {
